...
/// to a 32-bit **unsigned** integer suitable for use in WASM linear memory addressing.
...
///
/// # Returns
/// 32-bit **unsigned** integer representing the WASM memory address of the vector data
///
/// # Safety
...
///
/// # Panics
/// Panics if the pointer address exceeds the WASM 32-bit address space (> 4GB).
///
/// # Usage
...
///
/// # Note
/// Changed from `i32` to `u32` to fix pointer wraparound bug. WASM linear memory
/// addresses are always unsigned (0 to 4GB), and using signed `i32` could cause
/// negative wraparound for allocations in the upper 2GB range.
...
///
/// # Returns
/// 32-bit **unsigned** integer representing the WASM memory address 4 bytes into the vector
///
/// # ArrayBuffer Layout
...
///
/// # Returns
/// 32-bit **unsigned** integer pointer to the data in WASM memory (pointing past length prefix)
///
/// # Memory Management
...
pub fn export_bytes(v: Vec<u8>) -> u32 {
    let response: Vec<u8> = to_arraybuffer_layout(&v);
    let ptr = Box::leak(Box::new(response)).as_mut_ptr() as usize;
    assert!(ptr + 4 <= u32::MAX as usize,
            "Exported bytes pointer beyond WASM address space at 0x{:x}", ptr + 4);
    (ptr + 4) as u32
}
