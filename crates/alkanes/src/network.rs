...
/// Deploy futures from blocks generated by generatefuture
/// 
/// When generatefuture creates a block, the coinbase has special outputs.
/// This function detects those blocks and deploys a future contract at [31:height].
pub fn deploy_futures_from_protostones(block: &Block, height: u32) -> Result<()> {
    ...
    // generatefuture creates coinbase with 3 outputs: payment + witness commitment + protostone
    // Regular coinbase typically has 1-2 outputs
    // This is a simple heuristic that should work for regtest
    if coinbase.output.len() >= 3 {
        ...
        let mut future_ptr =
            IndexPointer::from_keyword("/alkanes/").select(&future_id.into());
