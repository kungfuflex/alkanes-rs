...
pub fn ptr_to_vec(_ptr: i32) -> Vec<u8> {
    // In native builds, we ignore the pointer and take the vec from the channel.
    // The slice is also stripped of its length prefix here.
    let buf = crate::wasm::take_native_ptr().expect("Pointer channel is empty. This is a bug in the native shim.");
    buf[4..].to_vec()
}
