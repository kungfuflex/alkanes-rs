...
fn is_delegate_invoke_mint(event: &JsonValue) -> bool {
    ...
    if let Some(opcode_hex) = inputs.get(0).and_then(|v| v.as_str()) {
        // opcode 0x1 indicates mint (add liquidity)
        return hex_to_u128(opcode_hex) == Some(1);
    }
    false
}

/// Detects pool mint (add liquidity) events and returns fully shaped rows ready for DB insert
pub async fn index_pool_mints_for_block(
    db: &PgPool,
    block_height: i32,
    ...
    // Preload decoded protostones for minter address extraction
    ...
    for (_txid, _tx_idx, _timestamp, _tx_json, events) in results.iter() {
        for ev in events {
            if !is_delegate_invoke_mint(ev) { continue; }
    ...
    for (txid, tx_idx, timestamp, _tx_json, events) in results.iter() {
        ...
        for (i, ev) in ordered.iter().enumerate() {
            ...
            // Pick matching return where LP out > incoming LP and both tokens net-positive in
            ...
            for j in (i+1)..ordered_events.len() {
                ...
                let lp_out = calculate_token_total(&outgoing, &pool_block, &pool_tx);
                ...
                let t0_net = t0_in_total.saturating_sub(t0_out_total);
                let t1_net = t1_in_total.saturating_sub(t1_out_total);
                if t0_net == 0 || t1_net == 0 { continue; }
                // Prefer minimal token outs, latest on tie
                let score = (t0_out_total, t1_out_total, j);
    ...
    if !mint_rows.is_empty() {
        // Delete then insert per txids
        let mut dbtx = db.begin().await?;
        replace_pool_mints(&mut dbtx, &txids, &mint_rows).await?;
        dbtx.commit().await?;
    }
    info!(mints = mint_rows.len(), "indexed pool mints for block");
    Ok(())
}
