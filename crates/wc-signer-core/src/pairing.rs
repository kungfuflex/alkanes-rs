...
/// Mint a fresh `frtun1<bech32m>.peer` peer name. The bridge validates
/// strict bech32m with HRP `frtun` — hex bodies are rejected with a
/// connection reset during the listen handshake. Body is 20 random
/// bytes (matches the TS sender's `fp.generatePeer()` shape).
pub fn generate_cli_peer_name() -> String {
    use bech32::{Bech32m, Hrp};
    let mut buf = [0u8; 20];
    OsRng.fill_bytes(&mut buf);
    let hrp = Hrp::parse("frtun").expect("static HRP");
    let encoded = bech32::encode::<Bech32m>(hrp, &buf).expect("bech32m encode");
    format!("{}.peer", encoded)
}
...
mod tests {
    ...
    fn cli_peer_name_shape() {
        ...
        assert!(p.starts_with("frtun1"), "got {}", p);
        assert!(p.ends_with(".peer"), "got {}", p);
        // bech32m of HRP "frtun" + 20-byte payload:
        //   HRP "frtun"    = 5 chars
        //   separator "1"  = 1 char
        //   data           = 32 chars (20-byte payload bech32-encoded)
        //   checksum       = 6 chars
        //   ".peer" suffix = 5 chars
        // Total = 5 + 1 + 32 + 6 + 5 = 49 chars
        assert_eq!(p.len(), 49, "got len {} for {}", p.len(), p);
        // Bech32 alphabet excludes b, i, o, 1 (after the separator).
        let body = &p[6..p.len() - 5];
        for c in body.chars() {
            assert!(!"bio".contains(c), "non-bech32 char {} in {}", c, p);
        }
    }
}
