...
pub enum WcError {
    ...
    Timeout(Duration),
    /// Fast-fail ping gate found the peer unreachable. Surfaced inside
    /// `send_request` before the heavy app-layer request goes out.
    /// Only emitted with the `icmp` feature on.
    #[cfg(feature = "icmp")]
    #[error("peer unreachable: {0}")]
    PeerUnreachable(String),
}

impl From<crate::transport::TransportError> for WcError {
    fn from(e: crate::transport::TransportError) -> Self {
        WcError::Transport(e.to_string())
    }
}

impl From<crate::storage::StorageError> for WcError {
...
impl<T: WalletTransport + 'static, S: SessionStorage + 'static> WalletConnectSigner<T, S> {
    ...
    async fn send_request(&self, req: &Plaintext) -> Result<Plaintext, WcError> {
        ...
        // 2a. Fast-fail reachability probe (icmp feature).
        //
        // Before we ship the (possibly hefty) encrypted request, ICMP
        // the peer with a 5s window. If they're not listening — which
        // happens routinely on a flaky FCM wake or a stale session —
        // we get a clear `PeerUnreachable` instead of stalling for
        // the full app-layer response timeout.
        //
        // With the feature off this block compiles out entirely; the
        // wire shape is the pre-icmp byte stream and there's nothing
        // to ping. Both peers must agree on the flag.
        #[cfg(feature = "icmp")]
        {
            use std::time::Duration;
            stream
                ...
                .await
                .map_err(|e| WcError::PeerUnreachable(format!("ping: {e}")))?;
        }
...
// --- timer shims -----------------------------------------------------
// On native (any non-wasm32 target) with the `native-time` feature on
// (default), use tokio::time. On wasm32, always use gloo-timers. If a
// non-wasm32 consumer disables `native-time`, the sleep/timeout become
// no-ops and the caller is responsible for bounding requests
// externally — this is the path the codec-only signer ships to a
// wasi-style host that doesn't link tokio.

#[cfg(all(not(target_arch = "wasm32"), feature = "native-time"))]
async fn sleep_compat(d: Duration) {
    tokio::time::sleep(d).await;
}

#[cfg(all(not(target_arch = "wasm32"), not(feature = "native-time")))]
async fn sleep_compat(_d: Duration) {
    // No timer available — return immediately. Native consumers should
    // enable `native-time` to get a real backoff.
}
...
async fn tokio_timeout<F: std::future::Future>(
    ...
    // Race the caller future against a gloo-timers sleep. If the timer
    // wins, return WcError::Timeout(d) to match the native shim shape
    // exactly — the JS-side Promise.race guard in bindgen-cli is then
    // redundant.
    use futures::future::{select, Either};
    use futures::pin_mut;
    let sleep = gloo_timers::future::TimeoutFuture::new(d.as_millis() as u32);
    pin_mut!(f, sleep);
    match select(f, sleep).await {
        Either::Left((v, _)) => Ok(v),
        Either::Right((_, _)) => Err(WcError::Timeout(d)),
    }
}
