...
#[test]
#[ignore = "WIP: init_pool reverts 'expects 20 arguments' though the PROXY provably receives a correct cellpack. Verified by dumping the delivered inputs: [0, 2,1, 32,0, 1e10,1e10, 400000, 1.45e14, 2e6, 8e6, 2.3e14, 2e12, 1.46e14, 5e9, 600, 6.4164e22, 2,2, 2,2, then 13 padding zeros] -- 34 values, so opcode=inputs[0]=0 and args=inputs[1..] gives len 33, comfortably past the >=20 check, with every argument in its documented slot. Confirmed with --test-threads=1, so this is NOT log interleaving from the init_refuses_* tests (I assumed that first; it was wrong). RULED OUT: the arg count as sent; DeployPair::call_only vs new(vec![], ..) (same struct); the proxy dispatch (the revert is the POOL's own message, so the delegatecall lands); and opcode stripping -- Upgradeable::fallback forwards context.inputs VERBATIM and the pool reads inputs[0] as its opcode (cryptoswap-pool/src/lib.rs:114-118), which is correct. REMAINING SUSPECT: what survives the delegatecall boundary -- the proxy sees 34 inputs but the implementation may see a re-encoded, shorter vector. Instrument the implementation side next, not the caller side. Left ignored rather than deleted or force-passed: a green test here would be a lie about coverage of the live curve."]
