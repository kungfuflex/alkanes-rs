...
// SPIR-V stubs for memmem module
#[cfg(target_arch = "spirv")]
pub mod memmem {
    //! SPIR-V stub implementations for memmem functionality
    
    /// SPIR-V stub for Finder
    pub struct Finder;
    
    impl Finder {
        /// Create a new finder (SPIR-V stub)
        pub fn new(_needle: &str) -> Self {
            panic!("memmem operations not supported in SPIR-V")
        }
        
        /// Find needle in haystack (SPIR-V stub)
        pub fn find(&self, _haystack: &[u8]) -> Option<usize> {
            panic!("memmem operations not supported in SPIR-V")
        }
    }
    
    /// Find needle in haystack (SPIR-V stub)
    pub fn find(_haystack: &[u8], _needle: &[u8]) -> Option<usize> {
        panic!("memmem operations not supported in SPIR-V")
    }
    
    /// Create iterator for finding needle in haystack (SPIR-V stub)
    pub fn find_iter<'h>(_haystack: &'h [u8], _needle: &[u8]) -> FindIter<'h> {
        panic!("memmem operations not supported in SPIR-V")
    }
    
    /// SPIR-V stub for FindIter
    pub struct FindIter<'h> {
        _phantom: core::marker::PhantomData<&'h [u8]>,
    }
