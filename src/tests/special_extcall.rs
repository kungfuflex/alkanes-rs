...
#[wasm_bindgen_test]
fn test_special_extcall_number_contract_calls() -> Result<()> {
    clear();
    let block_height = 0;

    let create_get_num_contract_calls = Cellpack {
        target: AlkaneId { block: 1, tx: 0 },
        inputs: vec![108],
    };

    let get_num_contract_calls = Cellpack {
        target: AlkaneId { block: 2, tx: 1 },
        inputs: vec![108],
    };
    ...
    let mut test_block = alkane_helpers::init_with_multiple_cellpacks_with_tx(
        ...
        .into(),
        [
            create_get_num_contract_calls.clone(),
            get_num_contract_calls.clone(),
            get_num_contract_calls.clone(),
            get_num_contract_calls.clone(),
            get_num_contract_calls.clone(),
            get_num_contract_calls.clone(),
