...
fn process_gpu_pipeline(input_data: &[u8], output_data: &mut [u8]) -> Result<()> {
    // Deserialize input shard
    let shard: GpuExecutionShard = serde_json::from_slice(input_data)?;
    ...
    // Serialize result to output buffer
    let result_json = serde_json::to_vec(&result)?;
    if result_json.len() <= output_data.len() {
        output_data[..result_json.len()].copy_from_slice(&result_json);
        Ok(())
    } else {
