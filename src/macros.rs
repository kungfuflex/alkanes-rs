...
macro_rules! metashrew_indexer {
    (
        ...
        static mut INDEXER_INSTANCE: Option<$crate::core_indexer::MetashrewIndexer<$indexer_type>> = None;
        ...
        pub extern "C" fn _start() {
            unsafe {
                if INDEXER_INSTANCE.is_none() {
                    ...
                    INDEXER_INSTANCE = Some($crate::core_indexer::MetashrewIndexer::new(indexer));
                }
                    ...
                    if let Some(indexer) = &INDEXER_INSTANCE {
                        let (_height, input_bytes) = match $crate::host::load_input() {
                            ...
                            Err(e) => {
                                ...
                                return $crate::core_view::return_view_result(&[]);
                            }
                        };
                        ...
                        match <$indexer_type>::$view_method(&indexer.get_indexer(), input) {
                            Ok(output) => {
                                ...
                                $crate::core_view::return_view_result(&bytes)
                            },
                            Err(e) => {
                                $crate::host::log(&format!("Error executing view function: {}", e));
                                $crate::core_view::return_view_result(&[])
                            }
                        }
                    } else {
                        $crate::core_view::return_view_result(&[])
...
macro_rules! metashrew_view {
    (
        ...
        pub extern "C" fn $name() -> i32 {
            match $crate::core_view::parse_view_input::<$input_type>() {
                Ok($input) => {
                    match (|| -> anyhow::Result<$output_type> $body)() {
                        Ok(output) => {
                            match serde_json::to_vec(&output) {
                                Ok(bytes) => $crate::core_view::return_view_result(&bytes),
                                Err(e) => {
                                    $crate::host::log(&format!("Error serializing output: {}", e));
                                    $crate::core_view::return_view_result(&[])
                                }
