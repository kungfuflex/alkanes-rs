...
| Sev | Where | Issue | Fix |
|-----|-------|-------|-----|
| HIGH | `crates/alkanes-support/src/storage.rs:21-24` | **FIXED + PoC.** `StorageMap::parse` reads an attacker `u32` key/value length and calls `consume_exact` which does `vec![0u8; n]` **before** checking bytes remain — a contract can return a response claiming ~4 GB, aborting the block. | Bounds each length + the entry count against remaining cursor bytes before allocating. PoC `src/tests/storage_alloc_poc.rs` (pre-fix: `capacity overflow` panic; post-fix: graceful `Err`). |
| HIGH | `src/lib.rs:434-441` | `AuxpowBlock::parse(...).unwrap()`, `consensus_decode::<Block>(...).unwrap()`, and `index_block(...).unwrap()` at the metashrew entry point turn any deterministic parse/index `Err` into a permanent wedge. **Not cleanly PoC-able**: the `_start` export is `#[cfg(not(test))]`, and the per-tx errors that reach `index_block` are already caught/rolled back internally, so no in-harness input drives a spurious `index_block` `Err`. Decision needed: a block that genuinely won't parse/index has no safe silent recovery (swallowing desyncs the index), so wedge-and-retry may be the intended metashrew behavior. | Left as-is pending a decision; if hardened, propagate a typed error and surface it rather than `.unwrap()`. |
| HIGH | `src/view.rs:1016-1067` | **FIXED + PoC.** The `simulate_protostones` view path enabled four global flags (`SKIP_PROTOSTONE_PERSISTENCE`, trace/final-balance/touched-storage collectors) with **no `Drop` guard**; the `seed_input_balances(...)?` early-return (or a panic) between enable and disable leaked `SKIP_PROTOSTONE_PERSISTENCE = true` into the next `index_block`, which then **skips `save_balances` + `clear_balances`** → index corruption. | `ViewCollectorGuard` RAII guard disables all four collectors on drop. PoC `src/tests/view_flag_leak_poc.rs` (pre-fix: flag leaks `true`; post-fix: no leak, follow-up block persists). |
| HIGH | `src/view.rs:955-968,814` | **FIXED + PoC.** `simulateprotostones` allocated `max(pointer)+1` dust `TxOut`s from an attacker `u32` pointer → multi-GB OOM on one RPC call. | Clamps `num_dust_outputs` to `MAX_SYNTH_DUST_OUTPUTS = 4096`; out-of-range pointers are then rejected gracefully by `process_message`. PoC `src/tests/view_dust_alloc_poc.rs` (pre-fix: capacity-overflow panic; post-fix: graceful response). |
| MED | `crates/protorune/src/protoburn.rs:204` | **FIXED + PoC.** `(cycle+1) % (self.max as i32)` with `self.max == 0` (zero protoburns but a leftover rune at the runestone/default output) is a `% 0` panic. `#[cfg(test)]`-gated in `index_protostones` today, so latent in production but LIVE in the test harness and the moment protoburn is enabled. | `Protoburns::process` no-ops on an empty burn set; `BurnCycle::next` returns `Err` on `max == 0`. PoC `src/tests/protoburn_divzero_poc.rs`. |
