...
mod tests {
    mod common;
    use common::{self as alkane_helpers, assert_return_context, assert_revert_context, clear, get_last_outpoint_sheet, simulate_parcel};
    use alkanes::message::AlkaneMessageContext;
    use alkanes::precompiled::{alkanes_std_auth_token_build, fr_btc_build};
    use alkanes::unwrap as unwrap_view;
    use alkanes::view;
    ...
    use alkanes::unwrap::{deserialize_payments, Payment};
    ...
    pub fn simulate_cellpack(height: u64, cellpack: Cellpack) -> Result<(ExtendedCallResponse, u64)> {
        let parcel = MessageContextParcel {
            ...
            sheets: Box::<BalanceSheet<alkanes::WasmHost>>::new(BalanceSheet::default()),
            txindex: 0,
            vout: 0,
            runtime_balances: Box::<BalanceSheet<alkanes::WasmHost>>::new(BalanceSheet::default()),
        };
        alkane_helpers::simulate_parcel(&parcel, u64::MAX)
    }
    ...
    fn wrap_btc() -> Result<(OutPoint, u64)> {
        ...
        WasmHost::default().index_block(&block, 1)?;

        let sheet = get_last_outpoint_sheet(&block)?;
        let binding = sheet.balances();
        let balance = binding.get(&fr_btc_id.clone().into()).unwrap();
    ...
    fn unwrap_btc(
        ...
        WasmHost::default().index_block(&block, height)?;
    ...
    fn test_fr_btc_wrap_incorrect_signer() -> Result<()> {
        ...
        // Create a block and index it
        block.txdata.push(wrap_tx.clone());
        WasmHost::default().index_block(&block, 880_001)?;
    ...
    fn test_last_block_updated_after_unwrap_fulfillment() -> Result<()> {
        ...
        WasmHost::default().index_block(&block2, height2)?;
        ...
        WasmHost::default().index_block(&block3, height3)?;
        ...
        alkanes::unwrap::update_last_block(height3 as u128)?;
