...
use alkanes::helpers as alkane_helpers;
use alkanes::std::alkanes_std_genesis_alkane_build;
use alkanes::vm::fuel::{FuelTank, TOTAL_FUEL_START};
...
fn test_genesis() -> Result<()> {
    ...
    alkanes::WasmHost::default().index_block(&test_block, block_height)?;
    ...
    WasmHost::default().index_block(&test_block2, block_height)?;
    ...
    let ptr = RuneTable::for_protocol(<AlkaneMessageContext as MessageContext<WasmHost>>::protocol_tag())
        .OUTPOINT_TO_RUNES
...
fn test_genesis_indexer_premine() -> Result<()> {
    ...
    WasmHost::default().index_block(&spend_block, 0)?;
    let new_outpoint = OutPoint {
        txid: spend_tx.compute_txid(),
        vout: 0,
    };
    let new_ptr = RuneTable::for_protocol(<AlkaneMessageContext as MessageContext<WasmHost>>::protocol_tag())
        .OUTPOINT_TO_RUNES
