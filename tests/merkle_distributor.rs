use crate::common::{self as alkane_helpers, assert_revert_context, clear, create_multiple_cellpack_with_witness_and_in, get_last_outpoint_sheet, get_sheet_for_outpoint, init_with_multiple_cellpacks_with_tx};
...
use ordinals::Edict as ProtostoneEdict;
...
fn helper_test_merkle_distributor(
    ...
    alkanes::WasmHost::default().index_block(&claim_block, block_height + 1)?;
...
fn test_merkle_distributor_admin_collect() -> Result<()> {
    ...
    alkanes::WasmHost::default().index_block(&spend_block, block_height)?;
