...
use ordinals::{Edict, Etching, Rune, Runestone};
...
#[cfg(test)]
use alkanes::std::alkanes_std_test_build;
...
pub fn get_sheet_for_outpoint(
    ...
    tx_num: usize,
    vout: u32,
) -> Result<BalanceSheet<alkanes::WasmHost>> {
    ...
    let sheet = load_sheet(&alkanes::WasmHost::default(), &ptr.get().as_ref().clone());
    println!(
...
pub fn get_sheet_for_runtime() -> BalanceSheet<alkanes::WasmHost> {
    let ptr = RuneTable::for_protocol(AlkaneMessageContext::protocol_tag()).RUNTIME_BALANCE;
    let sheet = load_sheet(&alkanes::WasmHost::default(), &ptr.get().as_ref().clone());
    sheet.expect("failed to load sheet")
}

pub fn get_lazy_sheet_for_runtime() -> BalanceSheet<alkanes::WasmHost> {
    let ptr = RuneTable::for_protocol(AlkaneMessageContext::protocol_tag()).RUNTIME_BALANCE;
    let sheet = BalanceSheet::default();
    sheet
}

pub fn get_last_outpoint_sheet(test_block: &Block) -> Result<BalanceSheet<alkanes::WasmHost>> {
            ...
            .map(|(i, cellpack_or_edict)| match cellpack_or_edict {
                ...
                CellpackOrEdict::Edict(edicts) => Protostone {
                    ...
                    edicts: edicts
                        ...
                        .map(|edict| {
                            let mut edict = edict;
                            edict.output = if etch { 5 + i as u32 } else { 4 + i as u32 };
...
use alkanes::precompiled::fr_btc_build;
use alkanes::vm::{instance::AlkanesInstance, runtime::AlkanesRuntimeContext};