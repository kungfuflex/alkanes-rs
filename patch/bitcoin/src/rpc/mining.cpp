...
static RPCHelpMan generatefuture()
{
    return RPCHelpMan{"generatefuture",
        ...
        "The coinbase transaction will include an OP_RETURN output with a Runestone\n"
        "containing a Protostone with cellpack [32, 0, 77] to enable future claiming.\n",
        {
        ...
        RPCExamples{
            HelpExampleCli("generatefuture", "\"bcrt1q...\"")
            + HelpExampleRpc("generatefuture", "\"bcrt1q...\"")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            // Decode address
            ...
            // Create coinbase script for the address
            CScript coinbase_script = GetScriptForDestination(destination);

            // Create block template with standard coinbase
            ...
            // Mine the block (generate exactly 1 block)
            uint64_t max_tries = DEFAULT_MAX_TRIES;
            std::shared_ptr<const CBlock> block_out;
            if (!GenerateBlock(chainman, std::move(block), max_tries, block_out, /*process_new_block=*/true)) {
                throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to generate block");
            }

            if (!block_out) {
                throw JSONRPCError(RPC_INTERNAL_ERROR, "Block generation failed");
            }
